<template>
    ...
    <div class="flex flex-col space-y-5 max-w-md mx-auto">
            ...
            <input type="text" name="username" id="username" placeholder="Username" v-model="username" :class="{'border-red-500': !usernameValid}" class="p-2 border rounded">
            <p v-if="!usernameValid" class="text-red-500 text-sm">Bitte geben Sie einen Benutzernamen ein.</p>
        
            <input type="password" name="Hash" id="hash" placeholder="Passwort (Hash)" v-model="password" :class="{'border-red-500': !passwordValid}" class="p-2 border rounded">
